...
<div class="wrap">
  ...
  <table>
    <thead>
      <tr><th>#</th><th>Rule</th><th>Phase</th><th>Type</th><th>Expected</th><th>Verdict</th></tr>
    </thead>
    <tbody>
      {% for e in evidence %}
      ...
      <tr class="detail" id="d{{ loop.index }}">
        <td colspan="6">{{ e.detail_block }}</td>
      </tr>
      {% endfor %}
    </tbody>
  </table>
  ...
  <ul class="chk">
    ...
    <li>
      <div class="rn">☐ {{ m.rule_description }}</div>
  ...
  <footer>
    ...
    Correlation via GraphQL firewallEventsAdaptive (CF-Ray ID) · Read-only, non-mutating pipeline.
  </footer>
</div>
<script>
  function toggle(id) {
...
def render_json(
    *,
    ...
    payload = {
        ...
        "disabled_rules": [
            {"description": d.description, "rule_type": d.rule_type}
            for d in disabled_rules
        ],
    }
